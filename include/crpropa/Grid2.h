...
/** Interpolate the grid at a given position */
	...
	T interpolate(const Vector3d &position) const {
		...
		if (typeid(get(ix,iy,iz)) == typeid(v000))
		{
			...
			b.setRThetaPhi(laenge,theta,phi);
		}
			//~ //Vektor zusammenbauen
			//~ if (typeid(v) == typeid(v000))
			//~ {v.setRThetaPhi(laenge,theta,phi);
			//~ }
			//~ //b.SetRThetaPhi(laenge,theta,phi);
		
		//~ if (typeid(get(ix,iy,iz)).name() == "Vector3")
		//~ {
			//~ double laenge = get(ix,iy,iz).getR();
		//~ }
		//~ 
		//~ double laenge = (get(ix,iy,iz)).getR()*fX*fY*fZ + (get(iX, iy, iz)).getR()*fx*fY*fZ + (get((ix, iY, iz)).getR()*fX*fy*fZ
		//~ + (get(ix,iy,iZ)).getR()*fX*fy*fZ + (get((iX,iy,iZ)).getR()*fx*fY*fz + (get(iX,iy,iZ)).getR()*fx*fY*fz + (get(ix, iY, iZ)).getR()*fX*fy*fz
		//~ + (get(iX,iY,iz)).getR()*fx*fy*fZ + (get((iX,iY,iZ)).getR()*fx*fy*fz;
		
		//~ double z = getZ(get(ix,iy,iz))*fX*fY*fZ + getZ(get(iX, iy, iz))*fx*fY*fZ + getZ(get(ix, iY, iz))*fX*fy*fZ
		//~ + getZ(get(ix,iy,iZ))*fX*fy*fZ + getZ(get(iX,iy,iZ))*fx*fY*fz + getZ(get(iX,iy,iZ))*fx*fY*fz + getZ(get(ix, iY, iZ))*fX*fy*fz
		//~ + getZ(get(iX,iY,iz))*fx*fy*fZ + getZ(get(iX,iY,iZ))*fx*fy*fz;
		//~ 
		//~ double theta = acos(z/laenge);
		//~ 
		//~ double y = getY(get(ix,iy,iz))*fX*fY*fZ + getY(get(iX, iy, iz))*fx*fY*fZ + getY(get(ix, iY, iz))*fX*fy*fZ
		//~ + getY(get(ix,iy,iZ))*fX*fy*fZ + getY(get(iX,iy,iZ))*fx*fY*fz + getY(get(iX,iy,iZ))*fx*fY*fz + getY(get(ix, iY, iZ))*fX*fy*fz
		//~ + getY(get(iX,iY,iz))*fx*fy*fZ + getY(get(iX,iY,iZ))*fx*fy*fz;
		//~ 
		//~ double x = getX(get(ix,iy,iz))*fX*fY*fZ + getX(get(iX, iy, iz))*fx*fY*fZ + getX(get(ix, iY, iz))*fX*fy*fZ
		//~ + getX(get(ix,iy,iZ))*fX*fy*fZ + getX(get(iX,iy,iZ))*fx*fY*fz + getX(get(iX,iy,iZ))*fx*fY*fz + getX(get(ix, iY, iZ))*fX*fy*fz
		//~ + getX(get(iX,iY,iz))*fx*fy*fZ + getX(get(iX,iY,iZ))*fx*fy*fz;
		//~ 
		//~ double phi = atan2(y,x);
		//~ 
		//~ //b zusammenbauen
